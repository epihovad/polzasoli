...
else
{
  ...
  <table width="100%" border="1" cellspacing="0" cellpadding="0" class="tab1">
    <tr>
      ...
      <th nowrap width="50%"><?=ShowSortPole($script,$cur_pole,$cur_sort,'Ссылка','link')?></th>
			<th nowrap>Фото <?=help('кол-во фото в данной рубрике')?></th>
  ...
  <?
	...
	if(sizeof($mas))
	{
        ...
        <? }?>
        <td><?=getCatUrl($row,true,$tbl,'gallery')?></td>
        <td align="center">
          <?
          if($count = getCountSub($id,$tbl,'gallery'))
          {
            ?><a href="" style="color:#090" onClick="RegSessionSort('gallery.php','gallery_catalog=<?=$id?>');return false;"><?=$count?></a><?
          }
          else
            echo '0';
          ?>
